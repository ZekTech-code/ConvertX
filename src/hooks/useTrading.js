...
export function useTrading(getPrice,) {
  const {trading, tradingRef, updateTradingState } = useAuth();
  const mountedRef = useRef(true);

  const startingBalance = trading?.startingBalance ?? 10000;
  const balance = trading?.balance ?? 10000;
  const positions = useMemo(() => trading?.positions ?? [], [trading?.positions]);
  const trades = useMemo(() => trading?.trades ?? [], [trading?.trades]);
  ...
  const clearTriggeredAlerts = useCallback(() => setLastTriggeredAlerts([]), []);
  ...
  const checkTPSL = useCallback(() => {
    ...
    for (const pos of curPositions) {
      ...
      if (pos.takeProfit) {
        if (pos.positionType === "long" && currentPrice >= pos.takeProfit) { shouldClose = true; }
        if (pos.positionType === "short" && currentPrice <= pos.takeProfit) { shouldClose = true; }
      }
      if (pos.stopLoss) {
        if (pos.positionType === "long" && currentPrice <= pos.stopLoss) { shouldClose = true; }
        if (pos.positionType === "short" && currentPrice >= pos.stopLoss) { shouldClose = true; }
      }
