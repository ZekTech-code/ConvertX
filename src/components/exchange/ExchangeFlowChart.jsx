...
// function getPairColor(pair, pairList) {
//   return PALETTE[pairList.indexOf(pair) % PALETTE.length];
// }



// Professional flat color palette (like the reference image)
const PALETTE = [
  "#E88F2B", // orange accent  (dominant)
  "#f59e0b", // amber
  "#f97316", // orange
  "#eab308", // yellow
  "#94a3b8", // slate
...
function ProfTooltip({ active, payload, label, darkMode: dm = true }) {
  if (!active || !payload?.length) return null;
  const total = payload.reduce((s, e) => s + (e.value || 0), 0);
  return (
    <div style={{
      background: dm ? "#000000" : "#ffffff",
      border: dm ? "1px solid rgba(255,255,255,0.1)" : "1px solid rgba(0,0,0,0.12)",
      borderRadius: 10,
      padding: "10px 14px",
      minWidth: 160,
      boxShadow: dm ? "0 8px 32px rgba(0,0,0,0.4)" : "0 8px 32px rgba(0,0,0,0.1)",
    }}>
      <p style={{ fontSize: 11, fontWeight: 700, color: dm ? "#94a3b8" : "#64748b", marginBottom: 6, letterSpacing: "0.04em" }}>
        {label}
      </p>
      {payload.map((entry, i) => (
        <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, marginBottom: 3 }}>
          <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ width: 8, height: 8, borderRadius: 2, background: entry.color, flexShrink: 0 }} />
            <span style={{ fontSize: 11, color: dm ? "#cbd5e1" : "#334155" }}>{entry.name}</span>
          </span>
          <span style={{ fontSize: 11, fontWeight: 700, fontFamily: "monospace", color: dm ? "#f8fafc" : "#000000" }}>
            {fmtVal(entry.value)}
          </span>
        </div>
      ))}
      {payload.length > 1 && (
        <div style={{ borderTop: dm ? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(0,0,0,0.08)", marginTop: 6, paddingTop: 6, display: "flex", justifyContent: "space-between" }}>
          <span style={{ fontSize: 10, color: dm ? "#64748b" : "#94a3b8", fontWeight: 700 }}>TOTAL</span>
          <span style={{ fontSize: 11, fontWeight: 900, fontFamily: "monospace", color: "#E88F2B" }}>{fmtVal(total)}</span>
        </div>
      )}
    </div>
  );
...
useEffect(() => {
  ...
  // Pair activity bar data — enriched with volume + avg rate per pair
        ...
        {!hasData ? (
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="w-full"
            >
            {/*TAB 1 — FLOW VOLUME Stacked bar (daily) + Donut (volume share)*/}
            {activeTab === "flow" && (
                  ...
                  return (
                    <div key={pair} className={`w-full border shadow-xl ${darkMode ? 'bg-black/95 border-white/10 shadow-black/30' : 'bg-white border-slate-200 shadow-slate-200/40'}`} style={{
                      ...
                      <div style={{
                        ...
                        {/* Subtle horizontal rule above chart */}
            ...
            {/*TAB 2 — NET POSITION Horizontal bar chart + Donut (absolute flow share)*/}
            {activeTab === "net" && (
              <div>
            ...
            {/*TAB 3 — PAIR ACTIVITY Vertical bar chart (count) + Donut (frequency share)*/}
            {activeTab === "frequency" && (
              <div style={{
                background: themeColors.bgCard,
                border: `1px solid ${themeColors.border}`,
            ...
            {/*TAB 4 — CUMULATIVE Stacked bar (daily) + Donut (total volume share)*/}
            ...
            {/*TAB 5 — RATE TREND 30-day live area / wave chart for the active pair */}
            {activeTab === "ratetrend" && (
              <div style={{
                background: themeColors.bgCard,
                border: `1px solid ${themeColors.border}`,
