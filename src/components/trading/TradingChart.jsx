...
export default function TradingChart({ asset, darkMode, currentPrice, pricesReady, onDataReady }) {
  ...
  useEffect(() => {
    ...
    const initialData = generateSyntheticCandles(price, timeframe);
  ...
  useEffect(() => {
    ...
    const loadRealData = async () => {
      ...
      candles = generateSyntheticCandles(currentPrice || 1, timeframe);
  ...
  }, [asset?.id, timeframe, digits, pricesReady]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const drift = (Math.random() - 0.5) * currentPrice * volatility;
      const newClose = +(prevClose + drift).toFixed(digits);
      ...
      try {
        if (ema9Ref.current) ema9Ref.current.update({ time: candleTime, value: +(newClose * 0.2 + prevClose * 0.8).toFixed(digits) });
        if (ema21Ref.current) ema21Ref.current.update({ time: candleTime, value: +(newClose * 0.1 + prevClose * 0.9).toFixed(digits) });
