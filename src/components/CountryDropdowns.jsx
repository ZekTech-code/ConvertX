...
// Helper component to render flag images safely from flagcdn
...
export function CountryDropdown({ value, onChange, error, disabled }) {
...
export function DialCodeDropdown({ value, onChange, disabled }) {
  const [isOpen, setIsOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const containerRef = useRef(null);
  ...
  // Find country matching the current dialCode
  const selectedCountry = useMemo(() => {
    if (!value) return null;
    return COUNTRIES.find((c) => c.dialCode === value);
  }, [value]);

  // Filter countries by dialCode or country name
  const filteredCountries = useMemo(() => {
    if (!searchQuery.trim()) return COUNTRIES;
