...
/*Standalone Field component (must be outside GetStarted to avoid "component created during  render" React error)*/
...
export default function GetStarted() {
  ...
  /*Brute-force lockout (Firestore-backed when Firebase is available, localStorage fallback otherwise)*/
  useEffect(() => {
    if (!email) return;
    let cancelled = false;
    ...
    const check = async () => {
      ...
      try {
        const { getLoginAttempts } = await import("../utils/security");
  ...
  return (
    <>
      {/*GLOBAL STYLES (theme-aware via CSS custom properties)*/}
      <style>{`
        ...
        /* Dot grid — adapts opacity per theme */
        .gs2-dots {
        ...
        /*Phone input with inline flag prefix*/
        .gs2-phone-wrap {
          position: relative;
          display: flex;
          align-items: center;
      ...
      `}</style>
      {/*CSS custom properties driven by darkMode*/}
      <div className="gs2-page gs2-bg relative overflow-x-hidden" style={darkMode ? {
        ...
        <main className="relative z-10 flex flex-col items-center px-4 py-8 sm:py-10">
            ...
            <div className="gs2-card p-6 sm:p-7">
              ...
              {verificationPending ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-3" noValidate>
                  ...
                  <AnimatePresence mode="wait">
                    {isSignUp && (
                      <motion.div key="signup-fields"
                        ...
                        {/* Row 3: Phone + Email — ONE LINE each */}
                        <div className="grid grid-cols-2 gap-2.5 gs2-grid2">

                          {/* Phone — inline flag + dial code prefix, live validation */}
                          {(() => {
                            ...
                            const liveResult = digits.length > 0
                              ...
                              : null;
                            // Show error: prefer submit error, then live typing error (only after 3+ digits so it's not too aggressive)
                          ...
                          {/* Email (signup) */}
                          <div>
                            <label className="gs2-label">Email Address</label>
                  ...
                  {/* Sign-in email (only shown when not signup) */}
                  {!isSignUp && (
          ...
          {/*MARKETING SECTION — below the form*/}
          <motion.div
