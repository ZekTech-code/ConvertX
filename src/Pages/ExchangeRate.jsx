...
// import LiveTradeBot from "../components/exchange/LiveTradeBot"; // ADD THIS IMPORT

// Flag Image helper component
...
/** Generate simulated historical data for a pair */
function generateHistoricalData(baseRate, pair, windowLabel) {
  const [from, to] = pair.split("/");
  // Deterministic seed from currency pair
  ...
  // How much lower the rate was at the START of the window (always positive → always rising to now)
  const driftPct =
    windowLabel === "24H" ?
      0.008 + (seed % 10) * 0.0003 // 0.8–1.1%
    : windowLabel === "7D" ?
      0.025 + (seed % 10) * 0.001 // 2.5–3.5%
    : 0.055 + (seed % 10) * 0.002; // 5.5–7.5%

  const startRate = baseRate * (1 - driftPct);
  const volatility = baseRate * 0.004;

  const history = Array.from({ length: points }, (_, i) => {
    // t goes 0→1: smooth upward base trend from startRate → baseRate
    const t = points > 1 ? i / (points - 1) : 1;
    const trendRate = startRate + (baseRate - startRate) * t;

    // Organic noise: deterministic sine waves at different frequencies
    const noise =
  ...
  // Force last point to exactly match the live rate
  if (history.length > 0) {
...
/** Derive trade-bot signal from trend analysis */
...
function PairSelector({ activePair, setActivePair, rates, darkMode }) {
  ...
  // const fromInfo = CURRENCY_INFO[activePair.from] ?? {};
  // const toInfo = CURRENCY_INFO[activePair.to] ?? {};
        ...
        {open && (
          <motion.div
            ...
            {PAIR_OPTIONS.map(({ from, to }) => {
              ...
              // const fi = CURRENCY_INFO[from] ?? {};
              // const ti = CURRENCY_INFO[to] ?? {};
              return (
                <button
                  key={`${from}/${to}`}
                  onClick={() => {
